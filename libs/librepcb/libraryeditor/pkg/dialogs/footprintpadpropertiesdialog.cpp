...
bool FootprintPadPropertiesDialog::applyChanges() noexcept
{
    try {
        ...
        cmd->setWidth(UnsignedLength(Length::fromMm(mUi->spbWidth->value())), false); // can throw
        cmd->setHeight(UnsignedLength(Length::fromMm(mUi->spbHeight->value())), false); // can throw
